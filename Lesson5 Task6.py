# Необходимо создать (не программно) текстовый файл, где каждая строка описывает учебный предмет и
# наличие лекционных, практических и лабораторных занятий по этому предмету и их количество. Важно,
# чтобы для каждого предмета не обязательно были все типы занятий. Сформировать словарь, содержащий
# название предмета и общее количество занятий по нему. Вывести словарь на экран.
#
# Примеры строк файла:
# Информатика: 100(л) 50(пр) 20(лаб).
# Физика: 30(л) — 10(лаб)
# Физкультура: — 30(пр) —
#
# Пример словаря:
# {“Информатика”: 170, “Физика”: 40, “Физкультура”: 30}

subs = {}
with open('./file_6.txt', 'r', encoding='utf-8') as file:
    for line in file:
        subject, lectures, practice, labs = line.split()
        lectures = lectures.split('(')[0] if lectures != '—' else 0
        practice = practice.split('(')[0] if practice != '—' else 0
        labs = labs.split('(')[0] if labs != '—' else 0
        # print(line, lectures, practice, labs)
        subs[subject] = int(lectures) + int(practice) + int(labs)
    print(f'Общее количество часов по предмету \n {subs}')
